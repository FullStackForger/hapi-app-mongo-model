...
externals.disconnect = function () {
	var promise = new Promise();
	
	if (externals.db) {
		externals.db.close(function () {
			promise.resolve();
		});		
	} else {
		promise.reject(new Error('there is no established connection'));
...
externals.register = function (options) {
	...
	model = Hoek.merge({
		_config: config,
		_getDB : function() { return externals.db; }
	}, helpers);

	// prepare collection API ( wrap monk collection methods )
	for (var property in Monk.Collection.prototype) {
		collectionApi[property] = (function(collectionName, methodName) {
			return function() {
				var collection = externals.db.get(collectionName);
				return collection[methodName].apply(collection, arguments);
			}
		})(config.collection, property);
	}
